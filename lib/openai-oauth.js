...
async function startOAuthLogin({
  ...
  originator = OPENAI_OAUTH_DEFAULTS.originator,
  ...
  activeLoginPromise = new Promise((resolve, reject) => {
    ...
    auth.searchParams.set('originator', originator);
    ...
    const server = http.createServer(async (req, res) => {
      try {
        ...
        finish(resolve, { success: true, tokenPath: savedPath, expiresIn: token.expires_in });
      } catch (error) {
    ...
    server.on('error', (error) => {
      ...
      const message = error?.code === 'EADDRINUSE'
        ? `OAuth callback port ${redirect.port || port} is already in use. Close the previous login window or restart the app, then try again.`
        : error.message;
