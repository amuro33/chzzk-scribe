...
function generateAssFromChats(chats, settings, videoStartTimestamp) {
    ...
    if (sortedChats.length > 0) {
        const firstChatTime = sortedChats[0].messageTime || sortedChats[0].time || sortedChats[0].timestamp;
        if (firstChatTime && (!startTs || firstChatTime < startTs)) {
            console.log(`[ASS] Adjusting startTs from ${videoStartTimestamp} to ${firstChatTime}`);
            startTs = firstChatTime;
        }
    }
...
`;
    ...
    for (const chat of sortedChats) {
        ...
        const messageTime = chat.messageTime || chat.time || chat.timestamp;
        ...
        let relativeMs = messageTime - startTs;
