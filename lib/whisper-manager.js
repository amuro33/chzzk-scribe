            ...
            if (fs.existsSync(pythonExe)) {
                ...
                if (hasTorch) {
                    try {
                        ...
                        const torchVersion = execSync(`"${pythonExe}" -c "import torch; print(torch.__version__)"`, { encoding: 'utf-8' }).trim();
                        const isCpuVersion = torchVersion.includes('+cpu');
                        
                        if (isCpuVersion) {
                            // GPU가 있는지 확인
                            const hasGpu = await this.detectNvidiaGPU(pythonExe);
                            if (hasGpu) {
                                console.log('[WhisperManager] GPU가 있지만 CPU 버전 PyTorch가 설치되어 있습니다. GPU 버전으로 업그레이드합니다.');
                                needsGpuTorch = true;
                            }
                        }
                    } catch (e) {
                ...
                // 패키지가 없거나 GPU 업그레이드가 필요하면 자동 설치 시도
                if (!isEngineReady) {
                    console.log('[WhisperManager] Python 패키지 설치/업그레이드 필요. 자동 설치 시작...');
                    await this.installPythonPackages(pythonExe, !hasTorch || needsGpuTorch, !hasFasterWhisper);
            ...
            if (needTorch) {
                if (hasNvidiaGPU) {
                    ...
                    try {
                        ...
                        execSync(`"${pythonExe}" -m pip uninstall -y torch`, { stdio: 'inherit' });
                    } catch (e) {
                        // 이미 없으면 무시
                    }
                        ...
                        child.on('close', (code) => {
                            if (code === 0) {
                                ...
                                if (needFasterWhisper) {
                                    ...
                                    const child2 = spawn(pythonExe, ['-m', 'pip', 'install', 'faster-whisper'], { stdio: 'inherit' });
                                    child2.on('close', (code2) => {
                                        if (code2 === 0) {
                                            console.log('[WhisperManager] ✅ Faster-Whisper 설치 완료');
                                            resolve();
                                        } else {
                                            reject(new Error('Faster-Whisper 설치 실패'));
                                        }
                                    });
                                } else {
                                    resolve();
                                ...
                                reject(new Error('PyTorch 설치 실패'));
                            }
                        });
                    });
                } else {
                ...
                return new Promise((resolve, reject) => {
                    console.log('[WhisperManager] Faster-Whisper 설치 중...');
                    const child = spawn(pythonExe, ['-m', 'pip', 'install', 'faster-whisper'], { stdio: 'inherit' });
