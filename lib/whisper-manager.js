...
`
                                ...
                                if (needFasterWhisper) {
                                    ...
                                    child2.on('close', (code2) => {
                                        if (code2 === 0) {
                                            ...
                                            this.patchCtranslate2(pythonExe).then(resolve);
                                        } else {
        ...
        return new Promise((resolve, reject) => {
            ...
            const installTorch = () => {
                return new Promise((resolveTorch, rejectTorch) => {
                    let torchArgs;
                    if (useGpu) {
                        console.log('[WhisperManager] Step 1/2: PyTorch 최신 버전 (CUDA 12.9 + cuDNN) 설치 중...');
                        if (onProgress) onProgress(0.05, 'PyTorch + cuDNN 다운로드 중...');
                        torchArgs = [
                            '-m', 'pip', 'install',
                            '--force-reinstall',
                            '--no-cache-dir',
                            '--upgrade',  // 최신 버전으로 업그레이드
                            'torch', 'torchvision', 'torchaudio',  // cuDNN 포함된 전체 패키지
                            '--index-url', 'https://download.pytorch.org/whl/cu121'  // CUDA 12.1 (안정 버전)
                        ];
                    } else {
                    ...
                    let installingPackages = []; // 설치할 패키지 목록
                    let progressInterval = null; // 진행률 업데이트 인터벌
                        ...
                        for (const line of lines) {
                            ...
                            if (line.includes('Collecting')) {
                                ...
                                if (match) {
                                    ...
                                    message = `패키지 수집: ${pkgName}`;
                                    if (onProgress) onProgress(currentProgress / 100, message);
                                }
                            } else if (line.includes('Downloading')) {
                                // 파일명과 크기 추출
                                const fileMatch = line.match(/Downloading\s+([^\s]+)/);
                                const sizeMatch = line.match(/\((\d+\.?\d*\s*[KMG]?B)/);

                                if (fileMatch) {
                                    const fileName = fileMatch[1].split('/').pop().split('-')[0];
                                    const size = sizeMatch ? sizeMatch[1] : '';
                                    message = `다운로드: ${fileName}${size ? ` (${size})` : ''}`;
                                } else {
                                ...
                                // 설치할 패키지 목록 추출
                                const pkgMatch = line.match(/Installing collected packages:\s*(.+)/);
                                if (pkgMatch) {
                                    installingPackages = pkgMatch[1].split(',').map(p => p.trim());
                                    currentProgress = 45;

                                    // 시간 기반 진행률 업데이트 시작
                                    let pkgIndex = 0;
                                    progressInterval = setInterval(() => {
                                        if (pkgIndex < installingPackages.length) {
                                            const progress = 45 + Math.floor((pkgIndex / installingPackages.length) * 5);
                                            const message = `설치 중 (${pkgIndex + 1}/${installingPackages.length}): ${installingPackages[pkgIndex]}`;
                                ...
                                // 설치 완료된 패키지 목록
                                const pkgMatch = line.match(/Successfully installed\s*(.+)/);
                                if (pkgMatch) {
                                    const packages = pkgMatch[1].split(' ').filter(p => p.trim());
                                    message = `PyTorch 설치 완료! (${packages.length}개 패키지)`;
                                } else {
            ...
            const installWhisper = () => {
                return new Promise((resolveWhisper, rejectWhisper) => {
                    ...
                    let whisperInstallingPackages = []; // 설치할 패키지 목록
                    let whisperProgressInterval = null; // 진행률 업데이트 인터벌
                    ...
                    whisperChild.stdout.on('data', (data) => {
                        ...
                        for (const line of lines) {
                            ...
                            if (line.includes('Collecting')) {
                                ...
                                const pkgMatch = line.match(/Installing collected packages:\s*(.+)/);
                                if (pkgMatch) {
                                    whisperInstallingPackages = pkgMatch[1].split(',').map(p => p.trim());
                                    currentProgress = 90;

                                    // 시간 기반 진행률 업데이트 시작
                                    let pkgIndex = 0;
                                    whisperProgressInterval = setInterval(() => {
                                        if (pkgIndex < whisperInstallingPackages.length) {
                                            const progress = 90 + Math.floor((pkgIndex / whisperInstallingPackages.length) * 8);
                                            const message = `설치 중 (${pkgIndex + 1}/${whisperInstallingPackages.length}): ${whisperInstallingPackages[pkgIndex]}`;
                                            if (onProgress) onProgress(progress / 100, message);
                                            pkgIndex++;
                                        } else {
                                ...
                                if (pkgMatch) {
                                    const packages = pkgMatch[1].split(' ').filter(p => p.trim());
                                    message = `설치 완료! (${packages.length}개 패키지)`;
                                } else {
                                    message = '설치 완료!';
                                }
                                currentProgress = 98;
            ...
            installTorch()
                .then(() => installWhisper())
                .then(() => this.patchCtranslate2(pythonExe))
                .then((output) => {
    ...
    /**
     * ctranslate2 ROCm 경로 오류 패치
     * Windows/NVIDIA 환경에서 AMD ROCm 경로가 없어 발생하는 에러 방지
     */
    async patchCtranslate2(pythonExe) {
        console.log('[WhisperManager] ctranslate2 __init__.py 패치 중...');

        try {
            // python.exe 위치 기준으로 site-packages 경로 찾기
            const pythonDir = path.dirname(pythonExe);
            const sitePackages = path.join(pythonDir, 'Lib', 'site-packages');
            const initFile = path.join(sitePackages, 'ctranslate2', '__init__.py');

            if (fs.existsSync(initFile)) {
                // 안전한 버전의 __init__.py 내용
                const newContent = `import sys
...
if sys.platform == "win32":
    ...
    if add_dll_directory is not None:
        ...
        # ROCm 경로 존재 여부 확인 후 추가 (PATCHED)
        rocm_sdk_core = os.path.join(package_dir, "..", "_rocm_sdk_core", "bin")
        if os.path.exists(rocm_sdk_core):
            add_dll_directory(rocm_sdk_core)
            
        rocm_sdk_libs = os.path.join(package_dir, "..", "_rocm_sdk_libraries_custom", "bin")
        if os.path.exists(rocm_sdk_libs):
            add_dll_directory(rocm_sdk_libs)
...
`;
                fs.writeFileSync(initFile, newContent, 'utf-8');
                console.log('[WhisperManager] ✅ ctranslate2 패치 완료');
            } else {
                console.warn(`[WhisperManager] 패치 실패: 파일을 찾을 수 없음 (${initFile})`);
            }
        } catch (err) {
            console.error('[WhisperManager] 패치 중 오류 발생:', err);
        }
    }
}
