...
class VideoDownloader {
    ...
    start(jobId, url, savePath, fileName, resolution, cookies, maxFragments, downloadEngine, streamlinkPath, durationSeconds, bitrateBps, tempPath, thumbnailUrl) {
        ...
        let ffmpegPath = "";
        try {
            // Priority 1: @ffmpeg-installer/ffmpeg (smaller, essential build)
            const ffmpegInstaller = require('@ffmpeg-installer/ffmpeg');
            ffmpegPath = ffmpegInstaller.path;

            // Priority 2: Handle ASAR unpacking in production
            if (ffmpegPath && ffmpegPath.includes('app.asar') && !ffmpegPath.includes('app.asar.unpacked')) {
                ...
                if (fs.existsSync(unpackedPath)) {
                    ...
                    console.log(`[VideoDownloader] Using unpacked FFmpeg: ${ffmpegPath}`);
                }
            }

            // Priority 3: Check userData/bin (Legacy or manual fallback)
            if (!ffmpegPath || !fs.existsSync(ffmpegPath)) {
                const { app } = require('electron');
                const userDataPath = app.getPath('userData');
                const manualFfmpeg = path.join(userDataPath, 'bin', process.platform === 'win32' ? 'ffmpeg.exe' : 'ffmpeg');
                if (fs.existsSync(manualFfmpeg)) {
                    ffmpegPath = manualFfmpeg;
                }
            }
        } catch (e) {
            console.error("[VideoDownloader] FFmpeg path resolution failed:", e);
        ...
        const tempDownloadPath = path.join(workDir, fileName + ".tmp");
        ...
        spawnArgs.push(url);
        const quality = (resolution && resolution !== "best") ? resolution : "best";
        spawnArgs.push(quality);
        ...
        processInstance.on("close", (code) => {
            if (code === 0) {
                console.log(`[VideoDownloader] Download finished, starting faststart post-processing: ${jobId}`);
                job.status = "converting";

                // Spawn FFmpeg to move MOOV atom to the front (faststart)
                const ffmpegArgs = [
                    "-y",
                    "-i", tempDownloadPath,
                    "-c", "copy",
                ...
                job.process = ffmpegProcess; // Enable cancellation during conversion

                // CRITICAL: Unblock the pipe by reading stderr. FFmpeg uses stderr for logs.
                // If we don't read it, the buffer fills up and the process hangs (infinite conversion).
                ffmpegProcess.stderr.on('data', (data) => {
                    const log = data.toString().trim();
                    if (log) console.log(`[FFmpeg-${jobId}] ${log.split('\n')[0]}`);
                });
                ...
                ffmpegProcess.on("close", (ffmpegCode) => {
                    if (ffmpegCode === 0) {
                        try {
                            ...
                            console.error(`[VideoDownloader] Failed to delete temp file: ${tempDownloadPath}`, e);
                        }
                        job.status = "completed";
                        job.progress = 100;
                        job.downloadedSize = job.totalSize !== '-' ? job.totalSize : "Done";
                        job.filePath = finalFilePath;
                        console.log(`[VideoDownloader] Job completed successfully with faststart: ${jobId}`);
                    } else {
                        console.error(`[VideoDownloader] FFmpeg failed with code ${ffmpegCode}. Falling back to original file.`);
                        // Fallback: move the original file even if faststart failed
                        try {
                            if (fs.existsSync(tempDownloadPath)) {
                                if (fs.existsSync(finalFilePath)) fs.unlinkSync(finalFilePath);
                                fs.renameSync(tempDownloadPath, finalFilePath);
                                job.status = "completed";
                                job.filePath = finalFilePath;
                            } else {
                                job.status = "failed";
                                job.error = "FFmpeg conversion failed and temp file missing";
                            }
                        } catch (e) {
                            job.status = "failed";
                            job.error = `FFmpeg failed and recovery failed: ${e.message}`;
                        }
                    }
                });

                ffmpegProcess.on("error", (err) => {
                    console.error(`[VideoDownloader] FFmpeg process error:`, err);
                    // Fallback move
                    try {
                        if (fs.existsSync(tempDownloadPath)) {
                            if (fs.existsSync(finalFilePath)) fs.unlinkSync(finalFilePath);
    ...
    async deleteFiles(jobId) {
        ...
        const getCleanupPaths = (dir) => [
            ...
            path.join(dir, job.fileName + ".tmp"),
