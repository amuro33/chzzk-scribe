...
class VideoDownloader {
    ...
    start(jobId, url, savePath, fileName, resolution, cookies, maxFragments, downloadEngine, streamlinkPath, durationSeconds, bitrateBps, tempPath, thumbnailUrl) {
        ...
        let ffmpegPath = "";
        try {
            // Priority 1: Check userData/bin (Downloaded via Splash Screen)
            const { app } = require('electron');
            const userDataPath = app.getPath('userData');
            const downloadedFfmpeg = path.join(userDataPath, 'bin', process.platform === 'win32' ? 'ffmpeg.exe' : 'ffmpeg');

            if (fs.existsSync(downloadedFfmpeg)) {
                ffmpegPath = downloadedFfmpeg;
            } else {
                // Priority 2: Standard ffmpeg-static (for dev environment)
