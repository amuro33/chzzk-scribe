    ...
    restoreState() {
        try {
            if (fs.existsSync(this.stateFilePath)) {
                ...
                console.log(`[TaskProcessor] 저장된 작업 발견: 현재 작업 ${state.currentTask ? 1 : 0}개, 대기 작업 ${state.queue?.length || 0}개`);
                console.log(`[TaskProcessor] 프로그램 재시작으로 인해 모든 작업을 실패 처리합니다.`);
                
                // 프로그램이 종료되면서 중단된 작업들이므로 실패 처리
                const failedTasks = [];
                
                if (state.currentTask) {
                    console.log(`[TaskProcessor] 실패 처리: ${state.currentTask.vodTitle}`);
                    failedTasks.push(state.currentTask.id);
                }
                
                if (state.queue && state.queue.length > 0) {
                    console.log(`[TaskProcessor] ${state.queue.length}개 대기 작업 실패 처리`);
                    state.queue.forEach(task => failedTasks.push(task.id));
                }
