        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                ...
                this.currentProcess.stderr.on('data', (data) => {
                    ...
                    // GPU 감지 관련 중요 정보는 로그로도 표시
                    if (output.includes('GPU 감지') || output.includes('CUDA') || 
                        output.includes('GPU 가속') || output.includes('CPU 모드') ||
                        output.includes('PyTorch') || output.includes('감지된 GPU')) {
                        const lines = output.trim().split('\n');
                        lines.forEach(line => {
                            if (line.trim()) {
                                log(`[GPU] ${line.trim()}`, 'info');
                            }
                        });
                    }
